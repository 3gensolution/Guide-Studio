...
This script does not launch GuideStudio. It:
...
```text
C:\Users\<user>\AppData\Local\Temp\guidestudio-cursor-native-...
```
...
$env:CURSOR_TEST_OUTPUT_DIR = "C:\temp\guidestudio-cursor-test"
npm run test:cursor-native:win
```

## GuideStudio preview capture

```powershell
npm run capture:guidestudio-preview
```

This script launches the real Electron app, injects a fixture video plus cursor sidecar data, opens the editor, captures frames from the actual GuideStudio preview UI, and encodes them into a WebM.
...
```powershell
$env:GUIDESTUDIO_PREVIEW_SKIP_BUILD = "true"
$env:GUIDESTUDIO_PREVIEW_FRAME_COUNT = "120"
$env:GUIDESTUDIO_PREVIEW_FPS = "30"
$env:GUIDESTUDIO_PREVIEW_OUTPUT_DIR = "C:\temp\guidestudio-preview"
npm run capture:guidestudio-preview
```

Useful files:

- `guidestudio-preview.webm`: video of the real GuideStudio editor preview
...
- whether the real GuideStudio preview renders the same cursor behavior as the diagnostic pipeline
...
Build GuideStudio's helper locally:
...
For local diagnostics with another compatible helper, point GuideStudio at that executable:

```powershell
$env:GUIDESTUDIO_WGC_CAPTURE_EXE = "C:\path\to\wgc-capture.exe"
npm run build-vite
npm run dev
```
