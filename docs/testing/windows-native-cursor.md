...
This script does not launch Guide Studio. It:
...
```text
C:\Users\<user>\AppData\Local\Temp\guide-cursor-native-...
```
...
$env:CURSOR_TEST_OUTPUT_DIR = "C:\temp\guide-cursor-test"
npm run test:cursor-native:win
```

## Guide Studio preview capture

```powershell
npm run capture:guide-studio-preview
```

This script launches the real Electron app, injects a fixture video plus cursor sidecar data, opens the editor, captures frames from the actual Guide Studio preview UI, and encodes them into a WebM.
...
```powershell
$env:GUIDE_PREVIEW_SKIP_BUILD = "true"
$env:GUIDE_PREVIEW_FRAME_COUNT = "120"
$env:GUIDE_PREVIEW_FPS = "30"
$env:GUIDE_PREVIEW_OUTPUT_DIR = "C:\temp\guide-studio-preview"
npm run capture:guide-studio-preview
```

Useful files:

- `guide-studio-preview.webm`: video of the real Guide Studio editor preview
...
- whether the real Guide Studio preview renders the same cursor behavior as the diagnostic pipeline
...
Build Guide Studio's helper locally:
...
For local diagnostics with another compatible helper, point Guide Studio at that executable:

```powershell
$env:GUIDE_WGC_CAPTURE_EXE = "C:\path\to\wgc-capture.exe"
npm run build-vite
npm run dev
```
