...
const cursorHelperName = "guide-macos-cursor-helper";
...
// GUIDE_MAC_HELPER_ARCHS per matrix entry (accepts arm64, x64, or x86_64).
...
const archs = (process.env.GUIDE_MAC_HELPER_ARCHS ?? hostArch)
	.split(",")
	.map((a) => a.trim())
	.filter(Boolean)
	.map(normalizeArch);
