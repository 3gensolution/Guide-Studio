...
export const Shape3DCube = ({ startDelay = 0 }: { startDelay?: number }) => {
...
export const ShapeCircularProgress = ({
	percentage = 75,
	startDelay = 0,
}: {
	percentage?: number;
	...
	return (
		<AbsoluteFill style={{ background: C.black }}>
			...
			>
				{/* 背景円 */}
				<circle cx="150" cy="150" r="120" fill="none" stroke={C.gray[800]} strokeWidth="12" />
				{/* プログレス円 */}
				<circle
					cx="150"
					cy="150"
...
export const ShapeExplosion = ({ startDelay = 0 }: { startDelay?: number }) => {
	const frame = useCurrentFrame();
	const { fps } = useVideoConfig();
...
export const ShapeHelix = ({ startDelay = 0 }: { startDelay?: number }) => {
	const frame = useCurrentFrame();
		...
		<AbsoluteFill style={{ background: C.black, perspective: 800 }}>
			...
			>
				{Array.from({ length: points }).map((_, i) => {
					...
					const delay = i * 2;
					const pointProgress = lerp(
						frame,
						[startDelay + delay, startDelay + delay + 15],
						[0, 1],
						EASE.out,
					);
...
export const ShapeHexGrid = ({ startDelay = 0 }: { startDelay?: number }) => {
	const frame = useCurrentFrame();
	const { fps } = useVideoConfig();
...
export const ShapeMandala = ({ startDelay = 0 }: { startDelay?: number }) => {
	const frame = useCurrentFrame();
	const { fps } = useVideoConfig();
...
export const ShapeMorphing = ({ startDelay = 0 }: { startDelay?: number }) => {
	const frame = useCurrentFrame();

	// 0-1の周期的な値
	const cycle = (frame - startDelay) % 120;
	const morphProgress =
		cycle < 60 ? lerp(cycle, [0, 60], [0, 1], EASE.out) : lerp(cycle, [60, 120], [1, 0], EASE.out);
...
export const ShapeParticleField = ({
	particleCount = 60,
	startDelay = 0,
}: {
...
export const ShapeSpinningRings = ({ startDelay = 0 }: { startDelay?: number }) => {
	const frame = useCurrentFrame();
	const { fps } = useVideoConfig();
