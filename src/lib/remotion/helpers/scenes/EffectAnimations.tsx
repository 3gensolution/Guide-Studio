...
export const EffectChromaticAberration = ({ startDelay = 0 }: { startDelay?: number }) => {
...
export const EffectDepthOfField = ({ startDelay = 0 }: { startDelay?: number }) => {
	const frame = useCurrentFrame();
...
export const EffectDuotone = ({ startDelay = 0 }: { startDelay?: number }) => {
	const frame = useCurrentFrame();
...
export const EffectFilmGrain = ({ startDelay = 0 }: { startDelay?: number }) => {
	const frame = useCurrentFrame();
...
export const EffectGlow = ({ startDelay = 0 }: { startDelay?: number }) => {
	const frame = useCurrentFrame();
...
export const EffectKaleidoscope = ({ startDelay = 0 }: { startDelay?: number }) => {
	const frame = useCurrentFrame();
	const { fps } = useVideoConfig();
...
export const EffectLightLeak = ({ startDelay = 0 }: { startDelay?: number }) => {
	const frame = useCurrentFrame();
...
export const EffectMatrix = ({ startDelay = 0 }: { startDelay?: number }) => {
	const frame = useCurrentFrame();
						...
						{col.chars.map((c, charIndex) => (
							...
							>
								{charIndex === 0 || random(`matrix-show-${frame}-${colIndex}-${charIndex}`) > 0.02
									...
									: chars[
											Math.floor(
												random(`matrix-change-${frame}-${colIndex}-${charIndex}`) * chars.length,
											)
										]}
							</div>
						))}
...
export const EffectNoise = ({ startDelay = 0 }: { startDelay?: number }) => {
	const frame = useCurrentFrame();
...
export const EffectVHS = ({ startDelay = 0 }: { startDelay?: number }) => {
	const frame = useCurrentFrame();
