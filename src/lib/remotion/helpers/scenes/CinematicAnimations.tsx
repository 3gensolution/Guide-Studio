...
export const CinematicAction = ({ startDelay = 0 }: { startDelay?: number }) => {
	...
	const shake =
		hasImpact && frame < impactFrame + 10 ? (random(`action-shake-${frame}`) - 0.5) * 20 : 0;
	...
	return (
		...
		>
			...
			{/* レターボックス */}
			<div
				style={{ position: "absolute", top: 0, left: 0, right: 0, height: 60, background: C.black }}
			/>
			<div
				style={{
					position: "absolute",
...
export const CinematicAnime = ({ startDelay = 0 }: { startDelay?: number }) => {
	const frame = useCurrentFrame();
	const { fps } = useVideoConfig();
	...
	return (
		...
		>
			...
			{speedLines.map((line, i) => {
				const x = lerp(
					frame,
					[startDelay + line.delay, startDelay + line.delay + 15],
					[120, -20],
					EASE.out,
				);
...
export const CinematicDocumentary = ({ startDelay = 0 }: { startDelay?: number }) => {
	const frame = useCurrentFrame();
...
export const CinematicEpic = ({ startDelay = 0 }: { startDelay?: number }) => {
	const frame = useCurrentFrame();
...
export const CinematicHorror = ({ startDelay = 0 }: { startDelay?: number }) => {
	const frame = useCurrentFrame();
	...
	const glitchOffset =
		random(`horror-g-${frame}`) > 0.9 ? random(`horror-go-${frame}`) * 10 - 5 : 0;
...
export const CinematicMinimalEnd = ({ startDelay = 0 }: { startDelay?: number }) => {
	const frame = useCurrentFrame();
...
export const CinematicNoir = ({ startDelay = 0 }: { startDelay?: number }) => {
	const frame = useCurrentFrame();
...
export const CinematicRomance = ({ startDelay = 0 }: { startDelay?: number }) => {
	const frame = useCurrentFrame();
...
export const CinematicSciFi = ({ startDelay = 0 }: { startDelay?: number }) => {
	const frame = useCurrentFrame();
...
export const CinematicVintage = ({ startDelay = 0 }: { startDelay?: number }) => {
	const frame = useCurrentFrame();
