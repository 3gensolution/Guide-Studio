...
export const Logo3DRotate = ({ startDelay = 0 }: { startDelay?: number }) => {
...
export const LogoGlitch = ({ startDelay = 0 }: { startDelay?: number }) => {
	const frame = useCurrentFrame();
	const { fps } = useVideoConfig();
...
export const LogoLightTrail = ({ startDelay = 0 }: { startDelay?: number }) => {
	const frame = useCurrentFrame();
	...
	return (
		<AbsoluteFill style={{ background: C.black }}>
			...
			{[0, 1, 2, 3, 4].map((i) => {
				...
				const x = lerp(
					frame,
					[startDelay + trailDelay, startDelay + 40 + trailDelay],
					[-100, 50],
					EASE.out,
				);
				const opacity = lerp(
					frame,
					[startDelay + 30 + trailDelay, startDelay + 50 + trailDelay],
					[0.8, 0],
				);
...
export const LogoMaskReveal = ({ startDelay = 0 }: { startDelay?: number }) => {
	const frame = useCurrentFrame();
...
export const LogoMorph = ({ startDelay = 0 }: { startDelay?: number }) => {
	const frame = useCurrentFrame();
		...
		<AbsoluteFill style={{ background: C.gray[950] }}>
			...
			>
				{letters.map((letter, i) => {
					...
					const scale = letterProgress < 0.5 ? 1 - letterProgress : letterProgress;
...
export const LogoNeonSign = ({ startDelay = 0 }: { startDelay?: number }) => {
	const frame = useCurrentFrame();
          ...
          `,
			...
			>
				{letters.map((letter, i) => {
					...
					const letterFlicker = isOn
						? random(`neon-letter-${flickerSeed}-${i}`) > 0.05
							? 1
							: 0.2
						: 0;
...
export const LogoParticles = ({ startDelay = 0 }: { startDelay?: number }) => {
	const frame = useCurrentFrame();
	const { fps } = useVideoConfig();
...
export const LogoSplitScreen = ({ startDelay = 0 }: { startDelay?: number }) => {
	const frame = useCurrentFrame();
...
export const LogoStamp = ({ startDelay = 0 }: { startDelay?: number }) => {
	const frame = useCurrentFrame();
	const { fps } = useVideoConfig();
...
export const LogoStroke = ({ startDelay = 0 }: { startDelay?: number }) => {
	const frame = useCurrentFrame();
