		...
		<AbsoluteFill style={{ background: C.gray[950] }}>
			...
			>
				{chars.map((char, i) => {
					...
					const revealProgress = lerp(frame, [charDelay, charDelay + 15], [0, 100], EASE.out);
...
export const TextNeon = ({
	...
	const entryProgress = lerp(frame, [startDelay, startDelay + 30], [0, 1], EASE.out);
	const flicker =
		frame > startDelay + 30 ? 0.8 + random(`neon-${Math.floor(frame / 3)}`) * 0.2 : entryProgress;
...
export const TextScramble = ({
	...
	const getDisplayChar = (index: number, targetChar: string) => {
		...
		// ランダム文字を表示
		const randomIndex = Math.floor(random(`scramble-${frame}-${index}`) * chars.length);
					...
					{targetChars.map((char, i) => (
						<span
							...
							style={{
								...
								textShadow: getDisplayChar(i, char) === char ? "none" : `0 0 20px ${C.accent}`,
							}}
						>
