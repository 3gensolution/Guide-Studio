...
export const BackgroundAurora = ({ startDelay = 0 }: { startDelay?: number }) => {
...
export const BackgroundBokeh = ({ startDelay = 0 }: { startDelay?: number }) => {
	const frame = useCurrentFrame();
...
export const BackgroundFlowingGradient = ({ startDelay = 0 }: { startDelay?: number }) => {
	const frame = useCurrentFrame();
...
export const BackgroundGeometric = ({ startDelay = 0 }: { startDelay?: number }) => {
	const frame = useCurrentFrame();
...
export const BackgroundMeshGradient = ({ startDelay = 0 }: { startDelay?: number }) => {
	const frame = useCurrentFrame();
...
export const BackgroundNoiseTexture = ({ startDelay = 0 }: { startDelay?: number }) => {
	const frame = useCurrentFrame();
...
export const BackgroundPerspectiveGrid = ({ startDelay = 0 }: { startDelay?: number }) => {
	const frame = useCurrentFrame();
...
export const BackgroundRadial = ({ startDelay = 0 }: { startDelay?: number }) => {
	const frame = useCurrentFrame();
...
export const BackgroundWaves = ({ startDelay = 0 }: { startDelay?: number }) => {
	const frame = useCurrentFrame();
	...
	const generateWavePath = (offset: number, amplitude: number, frequency: number) => {
		...
		for (let x = 0; x <= width; x += 10) {
			const y =
				360 + Math.sin((x * frequency + (frame - startDelay) * 2 + offset) * 0.01) * amplitude;
			path += ` L ${x} ${y}`;
		}
	...
	return (
		<AbsoluteFill style={{ background: C.gray[950] }}>
			<svg width="100%" height="100%" style={{ position: "absolute" }} aria-hidden="true">
				{/* 波1（背面） */}
				<path d={generateWavePath(0, 40, 1)} fill={C.accent} opacity={0.3} />
				{/* 波2（中間） */}
				<path d={generateWavePath(100, 50, 1.5)} fill={C.secondary} opacity={0.4} />
				{/* 波3（前面） */}
				<path d={generateWavePath(200, 60, 2)} fill={C.tertiary} opacity={0.5} />
			</svg>
