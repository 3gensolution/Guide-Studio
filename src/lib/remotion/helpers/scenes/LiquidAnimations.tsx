...
export const LiquidBlob = ({ startDelay = 0 }: { startDelay?: number }) => {
...
export const LiquidCalligraphyInk = ({ startDelay = 0 }: { startDelay?: number }) => {
	const frame = useCurrentFrame();
...
export const LiquidFluidWave = ({ startDelay = 0 }: { startDelay?: number }) => {
	const frame = useCurrentFrame();
	const { fps, width, height } = useVideoConfig();
...
export const LiquidInkSplash = ({ startDelay = 0 }: { startDelay?: number }) => {
	const frame = useCurrentFrame();
	const { fps } = useVideoConfig();
...
export const LiquidMorphBlob = ({ startDelay = 0 }: { startDelay?: number }) => {
	const frame = useCurrentFrame();
	const { fps, width, height } = useVideoConfig();
...
export const LiquidOilSpill = ({ startDelay = 0 }: { startDelay?: number }) => {
	const frame = useCurrentFrame();
	const { fps, width, height } = useVideoConfig();
						...
						<svg width="400" height="400" viewBox="-200 -200 400 400" aria-hidden="true">
							<defs>
								...
								>
									...
									<stop
										offset="30%"
										stopColor={`hsl(${(layerHue + 40) % 360}, 80%, 50%)`}
										stopOpacity="0.6"
									/>
									<stop
										offset="60%"
										stopColor={`hsl(${(layerHue + 80) % 360}, 80%, 55%)`}
										stopOpacity="0.5"
									/>
									<stop
										offset="100%"
										stopColor={`hsl(${(layerHue + 120) % 360}, 80%, 45%)`}
										stopOpacity="0.4"
									/>
								</linearGradient>
							</defs>
...
export const LiquidPaintDrip = ({ startDelay = 0 }: { startDelay?: number }) => {
	const frame = useCurrentFrame();
	const { fps, width, height } = useVideoConfig();
...
export const LiquidSplatter = ({ startDelay = 0 }: { startDelay?: number }) => {
	const frame = useCurrentFrame();
	const { fps, width, height } = useVideoConfig();
...
export const LiquidSwirl = ({ startDelay = 0 }: { startDelay?: number }) => {
	const frame = useCurrentFrame();
	const { fps, width, height } = useVideoConfig();
...
export const LiquidWaterDrop = ({ startDelay = 0 }: { startDelay?: number }) => {
	const frame = useCurrentFrame();
	const { fps, width, height } = useVideoConfig();
