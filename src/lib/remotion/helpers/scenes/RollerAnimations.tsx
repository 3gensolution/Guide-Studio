...
export const Roller3DCarousel = ({ startDelay = 0 }: { startDelay?: number }) => {
			...
			>
				...
				>
					{words.map((word, i) => {
						...
						const isActive =
							Math.abs((rotation % 360) - itemAngle) < anglePerItem / 2 ||
...
export const RollerBlur = ({ startDelay = 0 }: { startDelay?: number }) => {
	const frame = useCurrentFrame();
	...
	const exitBlur =
		currentIndex >= finalIndex
			? 0
			: lerp(cycleT, [cycleDuration - 10, cycleDuration], [0, 20], EASE.smooth);
	...
	const exitOpacity =
		currentIndex >= finalIndex
			? 1
			: lerp(cycleT, [cycleDuration - 10, cycleDuration], [1, 0], EASE.smooth);
...
export const RollerCountdown = ({ startDelay = 0 }: { startDelay?: number }) => {
	const frame = useCurrentFrame();
	const { fps } = useVideoConfig();
	...
	const bgColor = isCountdown ? `rgb(${bgRed}, 10, 20)` : isStopping ? "#1a0a0a" : C.black;

	// パルス効果（カウントダウン中）
	const pulseScale = isCountdown ? 1 + 0.02 * Math.sin(t * 0.5) * (1 + urgency) : 1;
	...
	return (
		<AbsoluteFill
			style={{
				background: isStopping ? `radial-gradient(circle, #2a1a2e 0%, ${C.black} 100%)` : bgColor,
		...
		>
			...
			{isStopping && t < countdownDuration + spinDuration + 8 && (
				<AbsoluteFill
					style={{
						background: "#ff6b6b",
						opacity: lerp(t - (countdownDuration + spinDuration), [0, 8], [0.5, 0], EASE.out),
					}}
				/>
			)}
		</AbsoluteFill>
...
export const RollerDramaticStop = ({ startDelay = 0 }: { startDelay?: number }) => {
	const frame = useCurrentFrame();
	const { fps } = useVideoConfig();
					...
					<div style={{ transform: `translateY(${-offsetY}px)` }}>
						...
						<div
							style={{
								...
								justifyContent: "center",
								transform:
									displayIndex === finalIndex && isFinal
...
export const RollerDrum = ({ startDelay = 0 }: { startDelay?: number }) => {
	const frame = useCurrentFrame();
	const { fps } = useVideoConfig();
...
export const RollerFadeSlide = ({ startDelay = 0 }: { startDelay?: number }) => {
	const frame = useCurrentFrame();
	const { fps } = useVideoConfig();
...
export const RollerFlip = ({ startDelay = 0 }: { startDelay?: number }) => {
	const frame = useCurrentFrame();
	const { fps } = useVideoConfig();
...
export const RollerGlitch = ({ startDelay = 0 }: { startDelay?: number }) => {
	const frame = useCurrentFrame();
...
export const RollerGradientWave = ({ startDelay = 0 }: { startDelay?: number }) => {
	const frame = useCurrentFrame();
	...
	const currentIndex = Math.min(Math.floor(t / wordDuration), words.length - 1);
...
export const RollerLiquid = ({ startDelay = 0 }: { startDelay?: number }) => {
	const frame = useCurrentFrame();
...
export const RollerMaskSlide = ({ startDelay = 0 }: { startDelay?: number }) => {
	const frame = useCurrentFrame();
	...
	const maskProgress =
		currentIndex >= finalIndex
			? 0
			: lerp(cycleT, [cycleDuration - 15, cycleDuration], [0, 100], EASE.smooth);
				...
				<div style={{ position: "relative", overflow: "hidden" }}>
					...
					<div
						style={{
							...
							fontWeight: 900,
							color: C.white,
							clipPath: isTransitioning ? `inset(0 ${maskProgress}% 0 0)` : "none",
						}}
					>
						{words[currentIndex]}
					</div>
...
export const RollerMultiSlot = ({ startDelay = 0 }: { startDelay?: number }) => {
	const frame = useCurrentFrame();
	const { fps } = useVideoConfig();
		...
		<AbsoluteFill style={{ background: C.black }}>
			...
			>
				{slots.map((slot, slotIndex) => {
					...
					// 表示するワードとオフセット
					const wrappedScroll =
						((scrollY % (slot.words.length * wordHeight)) + slot.words.length * wordHeight) %
						(slot.words.length * wordHeight);
...
export const RollerOutlineHighlight = ({ startDelay = 0 }: { startDelay?: number }) => {
	const frame = useCurrentFrame();
	const { fps } = useVideoConfig();
		...
		<AbsoluteFill style={{ background: C.black }}>
			...
			>
				{[0, 1, 2, 3, 4, 5, 6].slice(0, rowCount).map((rowIdx) => {
					...
					// ハイライト時のスケール
					const scale = isHighlighted ? 1 + 0.05 * (1 - distance * 2) : 1 - distance * 0.02;
...
export const RollerPerspectiveStripes = ({ startDelay = 0 }: { startDelay?: number }) => {
	const frame = useCurrentFrame();
		...
		<AbsoluteFill style={{ background: C.black }}>
			...
			>
				{[0, 1, 2, 3, 4, 5, 6, 7, 8].slice(0, stripeCount).map((stripeIdx) => {
					...
					const wrappedY =
						((baseY % (stripeCount * stripeHeight * 2)) + stripeCount * stripeHeight * 2) %
						(stripeCount * stripeHeight * 2);
...
export const RollerScaleBounce = ({ startDelay = 0 }: { startDelay?: number }) => {
	const frame = useCurrentFrame();
	const { fps } = useVideoConfig();
...
export const RollerShuffle = ({ startDelay = 0 }: { startDelay?: number }) => {
	const frame = useCurrentFrame();
	const { fps } = useVideoConfig();
	...
	// 確定した文字数
	const confirmedCount = Math.min(Math.floor(t * charsPerFrame * 1.5), finalWord.length);
			...
			>
				...
				<div style={{ display: "flex", justifyContent: "center" }}>
					{finalWord.split("").map((char, pos) => {
						...
						if (!isConfirmed) {
							const randomIdx = Math.floor(pseudoRandom(t * 0.5 + pos * 100) * shuffleChars.length);
						...
						return (
							<div
								...
								style={{
									...
									textShadow: isConfirmed ? `0 0 ${20 * confirmProgress}px ${C.accent}99` : "none",
								}}
							>
								{displayChar}
							</div>
...
export const RollerSlotMachine = ({ startDelay = 0 }: { startDelay?: number }) => {
	const frame = useCurrentFrame();
	const { fps } = useVideoConfig();
...
export const RollerSlotReveal = ({ startDelay = 0 }: { startDelay?: number }) => {
	const frame = useCurrentFrame();
	const { fps } = useVideoConfig();
	...
	const finalScroll = isStopping ? scrollY + (targetScroll - scrollY) * springScroll : scrollY;
...
export const RollerSplitFlap = ({ startDelay = 0 }: { startDelay?: number }) => {
	const frame = useCurrentFrame();
	const { fps } = useVideoConfig();
	...
	// 現在の単語インデックス
	const currentWordIndex = Math.min(Math.floor(t / wordDuration), allWords.length - 1);
...
export const RollerTypewriter = ({ startDelay = 0 }: { startDelay?: number }) => {
	const frame = useCurrentFrame();
...
export const RollerVerticalList = ({ startDelay = 0 }: { startDelay?: number }) => {
	const frame = useCurrentFrame();
	const { fps } = useVideoConfig();
...
export const RollerWave = ({ startDelay = 0 }: { startDelay?: number }) => {
	const frame = useCurrentFrame();
				...
				<div style={{ display: "flex" }}>
					{currentWord.split("").map((char, i) => {
						...
						const exitY =
							currentIndex >= finalIndex
								? 0
								: lerp(cycleT, [exitStart, cycleDuration], [0, -30], EASE.smooth);
						const exitOpacity =
							currentIndex >= finalIndex
								? 1
								: lerp(cycleT, [exitStart, cycleDuration], [1, 0], EASE.smooth);

						const y = currentIndex >= finalIndex ? 0 : cycleT < cycleDuration - 10 ? enterY : exitY;
						const opacity =
							currentIndex >= finalIndex
								? 1
								: cycleT < cycleDuration - 10
									? enterOpacity
									: exitOpacity;
