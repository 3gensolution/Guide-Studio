...
export const DataBarChart = ({ startDelay = 0 }: { startDelay?: number }) => {
...
export const DataLineChart = ({ startDelay = 0 }: { startDelay?: number }) => {
	const frame = useCurrentFrame();
	...
	// エリアパス
	const areaPath = linePath + ` L ${getX(pointsToShow)} ${chartHeight} L 0 ${chartHeight} Z`;
					...
					{[0, 25, 50, 75, 100].map((value) => (
						<g key={`grid-${value}`}>
							<line
								...
								strokeDasharray="4 4"
							/>
							<text x={-40} y={getY(value) + 5} fill={C.gray[500]} fontSize={12} fontFamily={font}>
								{value}
							</text>
						</g>
					))}
...
export const DataPieChart = ({ startDelay = 0 }: { startDelay?: number }) => {
	const frame = useCurrentFrame();
	const { fps } = useVideoConfig();
...
export const DataProgressBars = ({ startDelay = 0 }: { startDelay?: number }) => {
	const frame = useCurrentFrame();
...
export const DataRanking = ({ startDelay = 0 }: { startDelay?: number }) => {
	const frame = useCurrentFrame();
	const { fps } = useVideoConfig();
	...
	return (
		<AbsoluteFill style={{ background: C.gray[950], padding: 60 }}>
			...
			{items.map((item, i) => {
				...
				const changeColor =
					item.change === "up" ? C.success : item.change === "down" ? C.danger : C.gray[500];
...
export const DataStatsCards = ({ startDelay = 0 }: { startDelay?: number }) => {
	const frame = useCurrentFrame();
	const { fps } = useVideoConfig();
...
export const DataTimeline = ({ startDelay = 0 }: { startDelay?: number }) => {
	const frame = useCurrentFrame();
	const { fps } = useVideoConfig();
