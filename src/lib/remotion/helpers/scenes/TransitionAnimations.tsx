				...
				return (
					<div
						...
						style={{
							...
							background: C.accent,
							transform: isVertical ? `scaleX(${progress / 100})` : `scaleY(${progress / 100})`,
...
export const TransitionBoxReveal = ({
	startDelay = 0,
	gridSize = 6,
}: {
	startDelay?: number;
	...
	return (
		<AbsoluteFill style={{ background: C.black }}>
			...
			>
				<div style={{ fontFamily: font, fontSize: 100, fontWeight: 800, color: C.white }}>GRID</div>
			</AbsoluteFill>
...
export const TransitionCircleWipe = ({
	startDelay = 0,
	originX = 50,
	originY = 50,
}: {
	...
	return (
		<AbsoluteFill style={{ background: C.black }}>
			...
			>
				<div style={{ fontFamily: font, fontSize: 60, color: C.gray[700] }}>OLD CONTENT</div>
			</AbsoluteFill>
...
export const TransitionDiagonalSlice = ({
	startDelay = 0,
	angle = 15,
}: {
	startDelay?: number;
...
export const TransitionGlitch = ({ startDelay = 0 }: { startDelay?: number }) => {
	const frame = useCurrentFrame();
...
export const TransitionLineSweep = ({
	startDelay = 0,
	lineCount = 5,
}: {
	startDelay?: number;
...
export const TransitionLiquidMorph = ({ startDelay = 0 }: { startDelay?: number }) => {
	const frame = useCurrentFrame();
...
export const TransitionShutter = ({
	startDelay = 0,
	bladeCount = 8,
}: {
	startDelay?: number;
	...
	const apertureSize = isClosed ? 0 : isOpening ? openProgress * 150 : (1 - closeProgress) * 150;
	...
	return (
		<AbsoluteFill style={{ background: C.black }}>
			...
			{!isOpening && (
				...
				>
					<div style={{ fontFamily: font, fontSize: 60, color: C.gray[700] }}>SCENE A</div>
				</AbsoluteFill>
			)}
...
export const TransitionZoomBlur = ({ startDelay = 0 }: { startDelay?: number }) => {
	const frame = useCurrentFrame();
