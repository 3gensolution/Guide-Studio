...
function getRelativePixelRate(width: number, height: number, frameRate: number): number {
	return (
		(Math.max(1, width) * Math.max(1, height) * Math.max(1, frameRate)) / BASELINE_PIXELS_PER_SECOND
	);
}
...
export function getExportBackpressureProfile(
	...
	const relativePixelRate = getRelativePixelRate(options.width, options.height, options.frameRate);
