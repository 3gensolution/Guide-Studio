...
function isSafeFiltergraphSpeed(speed: number): boolean {
	return Number.isFinite(speed) && speed >= MIN_FILTERGRAPH_SPEED && speed <= MAX_FILTERGRAPH_SPEED;
}
...
export function classifyEditedTrackStrategy(input: EditedTrackStrategyInput): EditedTrackStrategy {
	...
	if (
		input.sourceAudioFallbackPaths.some((audioPath) => audioPath !== input.primaryAudioSourcePath)
	) {
		return "offline-render-fallback";
	}

	if (
		buildEditedTrackSourceSegments(input.sourceDurationMs, input.trimRegions, input.speedRegions)
			.length === 0
	) {
		return "offline-render-fallback";
	}
