// @ts-nocheck — Copied from Recordly; IPC shapes differ from guide-studio
import type {
	AnnotationRegion,
	AudioRegion,
	AutoCaptionSettings,
...
export class ModernVideoExporter {
	...
	async export(): Promise<ExportResult> {
		...
		while (true) {
			...
			try {
				...
				const prefersNativeStaticLayoutBeforeBreeze = shouldPreferNativeStaticLayoutBeforeBreeze(
					runtimePlatform,
					backendPreference,
				);
				...
				} else if (backendPreference === "auto" && shouldPreferNativeAutoBackend(runtimePlatform)) {
				...
				this.backpressureProfile = getExportBackpressureProfile({
					encodeBackend: shouldDeferNativeEncoderStart || useNativeEncoder ? "ffmpeg" : "webcodecs",
				...
				console.log("[VideoExporter] Backpressure profile", {
					...
					encodeBackend: shouldDeferNativeEncoderStart || useNativeEncoder ? "ffmpeg" : "webcodecs",
					maxEncodeQueue: this.config.maxEncodeQueue ?? this.backpressureProfile.maxEncodeQueue,
				...
				if (shouldTryNativeStaticLayout && !useNativeEncoder) {
					...
					if (nativeVideoInfo) {
						...
						const effectiveDuration = this.getNativeStaticLayoutEffectiveDuration(nativeVideoInfo);
				...
				if (shouldTryNativeStaticLayout && !useNativeEncoder && !triedNativeStaticLayoutWithProbe) {
					const staticLayoutResult = await this.tryExportNativeStaticLayout(
						videoInfo,
						nativeAudioPlan,
						effectiveDuration,
	...
	private shouldRetryWithReadableFileSource(error: unknown): boolean {
		const resolvedError = this.encoderError ?? error;
		const message = resolvedError instanceof Error ? resolvedError.message : String(resolvedError);
	...
	private shouldUseNativeStaticLayoutTimelineMap(
		...
		return (
			trimRegions.length > 0 && !this.canUseNativeStaticTailTrim(videoInfo, effectiveDurationSec)
		);
	}
	...
	private buildNativeAudioPlan(videoInfo: DecodedVideoInfo): NativeAudioPlan {
		...
		const hasTimedSourceAudioFallback = sourceAudioFallbackPaths.some(
			(audioPath) => (this.config.sourceAudioFallbackStartDelayMsByPath?.[audioPath] ?? 0) > 0,
		);
		const localVideoSourcePath = this.getNativeVideoSourcePath();
	...
	private getNativeStaticLayoutSourceCrop(videoInfo: DecodedVideoInfo) {
		...
		const bottom = Math.min(sourceHeight, Math.max(top + 2, Math.ceil(cropBottom * sourceHeight)));
	...
	private getNativeStaticLayoutSkipReason(
		...
		return (
			this.getNativeStaticLayoutSkipReasons(audioPlan, videoInfo, effectiveDurationSec)[0] ?? null
		);
	}
	...
	private async resolveNativeStaticLayoutBackground(): Promise<NativeStaticLayoutBackground | null> {
		...
		if (wallpaper.startsWith("linear-gradient") || wallpaper.startsWith("radial-gradient")) {
			const materialized = await this.materializeNativeStaticLayoutGradientBackground(wallpaper);
			if (materialized) {
				return materialized;
			}
			this.nativeStaticLayoutBackgroundSkipReason =
	...
	private async resolveNativeBundledAssetPath(assetPath: string): Promise<string | null> {
		...
		try {
			...
			if (result?.success && result.files && !result.files.includes(decodeURIComponent(fileName))) {
	...
	private async getNativeStaticLayoutAudioOptions(audioPlan: NativeAudioPlan, totalFrames: number) {
		switch (audioPlan.audioMode) {
			case "none":
	...
	private getNativeStaticLayoutCursorSize(contentWidth: number) {
		...
		return (
			28 * (this.config.cursorSize ?? 3) * viewportScale * getCursorStyleSizeMultiplier(cursorStyle)
		);
	}
	...
	private getNativeStaticLayoutZoomTelemetry(
		...
		for (let frameIndex = 0; frameIndex < totalFrames; frameIndex += 1) {
			...
			const deltaMs = lastContentTimeMs !== null ? timeMs - lastContentTimeMs : frameDurationMs;
			...
			if (this.config.zoomClassicMode) {
				...
				appliedX = stepSpringValue(springX, projectedTransform.x, deltaMs, zoomSpringConfig);
				appliedY = stepSpringValue(springY, projectedTransform.y, deltaMs, zoomSpringConfig);
			}
	...
	private async tryExportNativeStaticLayout(
		...
		unsubscribeNativeProgress = window.electronAPI.onNativeStaticLayoutExportProgress?.(
			(progress) => {
				...
				if (!Number.isFinite(progress.currentFrame) || !Number.isFinite(progress.totalFrames)) {
					return;
				}
	...
	private async tryStartNativeVideoExport(): Promise<boolean> {
		...
		try {
			...
			console.warn(`[VideoExporter] ${NATIVE_EXPORT_ENGINE_NAME} encoder configure failed`, error);
			return false;
		}
	...
	private async finishNativeVideoExport(audioPlan: NativeAudioPlan): Promise<ExportResult> {
		...
		console.log(`[VideoExporter] Finalizing ${NATIVE_EXPORT_ENGINE_NAME} export`, {
			sessionId,
			audioMode: audioPlan.audioMode,
			editedTrackStrategy: audioPlan.audioMode === "edited-track" ? audioPlan.strategy : undefined,
		...
		const result = await this.measureFinalizationStage("nativeExportFinalizeMs", async () =>
			this.awaitWithFinalizationTimeout(
				window.electronAPI.nativeVideoExportFinish(sessionId, {
					...
					editedTrackSegments:
						audioPlan.audioMode === "edited-track" && audioPlan.strategy === "filtergraph-fast-path"
	...
	private async finalizeExportWithFfmpegAudio(
		...
		if (videoSource.mode === "stream") {
			...
			const result = await this.measureFinalizationStage("ffmpegAudioMuxMs", async () =>
				this.awaitWithFinalizationTimeout(
					window.electronAPI.muxExportedVideoAudioFromPath(videoSource.tempFilePath, muxOptions),
					"FFmpeg audio muxing",
					"audio",
				),
			);
	...
	private queueNativeWriteChunk(sessionId: string, chunk: Uint8Array): void {
		...
		if (
			this.pendingNativeWriteChunks.length >= ModernVideoExporter.NATIVE_WRITE_BATCH_MAX_CHUNKS ||
		...
		const writePromise = window.electronAPI
			...
			.then((writeResult) => {
				if (!writeResult.success && !this.cancelled) {
					throw new Error(writeResult.error || "Failed to write H.264 chunks to native encoder");
				}
			})
			.catch((error) => {
				if (!this.cancelled) {
	...
	private cleanup(): void {
		...
		this.lastFinalizationRenderProgress = INITIAL_FINALIZATION_PROGRESS_STATE.lastRenderProgress;
