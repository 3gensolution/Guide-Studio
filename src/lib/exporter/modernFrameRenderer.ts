...
function isCanvasRenderer(application: Application): boolean {
	...
	return Boolean(
		rendererName && (rendererName.includes("canvasrenderer") || rendererName.includes("canvas")),
	);
}
...
export class FrameRenderer {
		...
		if (cursorOverlayEnabled) {
			this.cursorOverlay = new PixiCursorOverlay({
				...
				clickEffectDurationMs:
					this.config.cursorClickEffectDurationMs ?? DEFAULT_CURSOR_CONFIG.clickEffectDurationMs,
			...
			typeof VideoFrame !== "undefined" && source instanceof VideoFrame
				...
				: typeof HTMLVideoElement !== "undefined" &&
					...
					? (() => {
							...
							staging.context.clearRect(0, 0, staging.canvas.width, staging.canvas.height);
							staging.context.drawImage(source, 0, 0, staging.canvas.width, staging.canvas.height);
							return staging.canvas;
						})()
					: source;
	...
	private hasActiveBlurAnnotations(timeMs: number): boolean {
		return (this.config.annotationRegions ?? []).some(
			(annotation) =>
				annotation.type === "blur" && timeMs >= annotation.startMs && timeMs <= annotation.endMs,
		);
	}
	...
	private drawCaptionOverlay(context: CanvasRenderingContext2D): void {
		if (!this.captionContainer?.visible || !this.captionSprite?.visible || !this.captionCanvas) {
			return;
		}
	...
	private async syncBackgroundFrame(timeSeconds: number): Promise<void> {
		if (this.backgroundForwardFrameSource) {
			...
			try {
				decodedFrame = await this.backgroundForwardFrameSource.getFrameAtTime(normalizedTargetTime);
			} catch (error) {
				console.warn(
			...
			const resolvedDecodedDuration = this.backgroundForwardFrameSource.getResolvedDurationSec();
			if (
				shouldLoop &&
	...
	private async loadBackgroundMediaElementSource(
		...
		try {
			...
			console.warn("[FrameRenderer] Unable to resolve video wallpaper fallback source:", error);
			return false;
		}
	...
	private updateWebcamOverlay(referenceTimeSeconds = this.currentVideoTime): void {
		...
		const expectedWebcamTargetTime = this.getExpectedWebcamTargetTimeSeconds(referenceTimeSeconds);
		...
		const webcamTimeDrift =
			measuredWebcamTime === null ? 0 : Math.abs(measuredWebcamTime - expectedWebcamTargetTime);
		...
		const aspectSourceWidth =
			liveSourceDimensions.width > 0 ? liveSourceDimensions.width : renderableWebcamSource.width;
		const aspectSourceHeight =
			liveSourceDimensions.height > 0 ? liveSourceDimensions.height : renderableWebcamSource.height;
	...
	private async renderSceneSample(
		...
		const webcamRenderTimeSeconds = Math.max(0, webcamTimeSecondsOverride ?? this.currentVideoTime);
	...
	private updateAnimationState(timeMs: number): number {
		...
		const deltaMs = this.lastContentTimeMs !== null ? timeMs - this.lastContentTimeMs : 1000 / 60;
