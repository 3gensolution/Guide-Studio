...
export class FrameRenderer {
	...
	async initialize(): Promise<void> {
		...
		if ((this.config.zoomMotionBlur ?? 0) > 0) {
			...
			this.videoContainer.filterArea = new Rectangle(0, 0, this.config.width, this.config.height);
	...
	async renderFrame(
		...
		if (!this.videoSprite) {
			...
			this.videoTextureSource ??= this.videoSprite.texture.source as unknown as VideoTextureSource;
				...
				const hookParams = {
					...
					cursor: temporalSnapshot.smoothedCursor
						? {
								cx: temporalSnapshot.smoothedCursor.cx,
								cy: temporalSnapshot.smoothedCursor.cy,
								interactionType: this.getCursorPosition(temporalSnapshot.cursorTimeMs)
									?.interactionType,
							}
	...
	private drawFrame(sceneTransform?: { scale: number; x: number; y: number }): void {
		...
		if (!insets) {
			...
			if (this.frameDraw) {
				...
				drawWithTransform(() => {
					ctx.drawImage(this.frameImage!, maskRect.x, maskRect.y, maskRect.width, maskRect.height);
				});
			}
			return;
		}
	...
	private drawWebcamOverlay(ctx: CanvasRenderingContext2D, width: number, height: number): void {
		...
		if (webcam.mirror) {
			...
			bubbleCtx.scale(-1, 1);
			bubbleCtx.drawImage(webcamFrameSource, sx, sy, sw, sh, drawX, drawY, drawWidth, drawHeight);
			bubbleCtx.restore();
		} else {
			bubbleCtx.drawImage(webcamFrameSource, sx, sy, sw, sh, drawX, drawY, drawWidth, drawHeight);
		}
