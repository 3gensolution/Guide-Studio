...
export class AssetBaseUnavailableError extends Error {
	constructor(relativePath: string) {
		super(`Asset base path is not available (resolving "${relativePath}")`);
...
export class UnsafeAssetPathError extends Error {
	constructor(resolvedPath: string) {
		super(`Resolved asset path escapes allowed prefix: ${resolvedPath}`);
		this.name = "UnsafeAssetPathError";
	}
}

/**
 * Synchronous version of getAssetPath. Uses the preload-injected
 * `window.electronAPI.assetBaseUrl` (a plain string, no IPC needed).
 * Safe to call from useMemo / render paths.
 */
export function getAssetPathSync(relativePath: string): string {
	const encodedRelativePath = encodeRelativeAssetPath(relativePath);

	// Reject path-traversal attempts (..) — check both the raw input
	// (catches percent-encoded `%2e%2e`) and the encoded output.
	const rawSegments = relativePath.split("/").map((s) => {
		try {
	...
	const encodedSegments = encodedRelativePath.split("/").map(decodeURIComponent);
	if (
		rawSegments.some((s) => s === ".." || s === ".") ||
		encodedSegments.some((s) => s === ".." || s === ".")
	) {
