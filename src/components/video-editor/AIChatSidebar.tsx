...
export function AIChatSidebar({
	...
	const sendMessage = useCallback(async () => {
		...
		try {
			...
			const responseText = await analyze(fullPrompt);

			if (!responseText) {
				const errorMsg: ChatMessage = {
					id: uuidv4(),
					role: "assistant",
					content: "Sorry, I could not process that request. Please check your AI settings.",
					timestamp: Date.now(),
				};
			...
			const { plainText, toolCalls } = parseToolCalls(responseText);
