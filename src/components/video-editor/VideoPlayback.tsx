...
function resolveClipAtTime(
	clips: VideoClip[],
	masterTimeMs: number,
): { videoPath: string; localTimeMs: number } | null {
	for (const clip of clips) {
		if (masterTimeMs >= clip.offsetMs && masterTimeMs < clip.offsetMs + clip.durationMs) {
			const elapsed = masterTimeMs - clip.offsetMs;
			const localTimeMs = clip.startMs + elapsed;
			return { videoPath: clip.sourceVideoPath, localTimeMs };
		}
	}
	return null;
}
...
const VideoPlayback = forwardRef<VideoPlaybackRef, VideoPlaybackProps>(
	(
		...
		// When clips are present and the master timeline position falls within a clip,
		// switch the video element's src and seek to the clip-local time.
		useEffect(() => {
			...
			const masterTimeMs = currentTime * 1000;
					...
					{pixiReady && videoReady && (
						...
						>
							<div
								...
								className="absolute rounded-md border border-[#6E6BFF]/80 bg-[#6E6BFF]/20 shadow-[0_0_0_1px_rgba(168,85,247,0.35)]"
