...
export function EditorEmptyState({ onVideoImported, onProjectOpened }: EditorEmptyStateProps) {
	...
	return (
		...
		>
			{/* Drop overlay */}
			{isDraggingOver && (
				<div className="pointer-events-none absolute inset-0 z-50 flex flex-col items-center justify-center rounded-xl border-2 border-dashed border-[#6E6BFF] bg-[#6E6BFF]/10">
					<Upload className="mb-3 h-10 w-10 text-[#6E6BFF]" />
					<p className="text-base font-semibold text-[#6E6BFF]">{te("emptyState.dropOverlay")}</p>
				</div>
			)}
			...
			<div className="relative flex flex-col items-center gap-6 px-6 text-center max-w-md">
				...
				<div className="flex gap-3 w-full max-w-sm">
					<button
						...
						className="flex-1 flex items-center justify-center gap-2 px-4 py-3 rounded-xl bg-[#6E6BFF] hover:bg-[#9333EA] active:bg-[#7E22CE] text-white font-medium text-sm transition-colors outline-none focus-visible:ring-2 focus-visible:ring-[#6E6BFF] focus-visible:ring-offset-2 focus-visible:ring-offset-[#1C1917]"
					>
