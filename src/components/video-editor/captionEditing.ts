...
function normalizeCaptionWords(cue: CaptionCue): CaptionCueWord[] {
	...
	return sourceWords
		...
		.map((word) => {
			const startMs = Math.max(cue.startMs, Math.min(cue.endMs - 1, Math.round(word.startMs)));
...
function distributeEditedTokensAcrossCueSegments(
	...
	const totalDuration = Math.max(
		1,
		segments.reduce((total, segment) => total + Math.max(1, segment.endMs - segment.startMs), 0),
	);
